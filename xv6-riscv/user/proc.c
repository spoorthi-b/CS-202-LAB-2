...
int TCount[NPROC]; // Store count of the ticks
int prog1id, prog2id, prog3id, pflag=0;

//LAB2
// Set tickets function
int createTickets(int n)
{
...
scheduler(void)
{
  ...
  for(;;)
  {
    //Lottery scheduler
	  ...
	  struct proc *p;
	  int totalTicks = 0 ;
	  for(p=proc;p<&proc[NPROC];p++)            //iterating over process list
	  {
		if(p->state==RUNNABLE){
		  totalTicks+= p->tickets;                //getting total number of ticketsß
		}
	  }
	  int winner = random_at_most(totalTicks);  //picking a ticket number randomly
	  ...
	  for(p=proc;p<&proc[NPROC];p++)
	  {
	    if(p->state==RUNNABLE)
		  temp+=p->tickets; 
	    if(temp>winner)                         //if the number of tickets is higher than the selected ticket number
	    {
		  acquire(&p->lock); 
		  p->state = RUNNING;                     //start executing
		  c->proc = p;
		  TCount[p->pid]+=1; 
